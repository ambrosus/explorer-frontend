          ...
          <div className="addresses_table">
            ...
            {renderData && renderData.data && renderData.data.length ? (
              !isQueryContracts
                ...
                }) : renderData.data.filter((acc: any) => acc.isContract === true).map((account: Account, index: number) => {
                  return (
                    <AddressesBody
                      key={account._id}
                      lastCardRef={
                        account && renderData.data.filter((acc: any) => acc.isContract === true).length - 1 === index
                          ? ref
                          : null
                      }
                      isContract={account.isContract}
