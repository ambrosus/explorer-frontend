...
const Method = ({index, method}: any) => {
  ...
  const contractCall = async (method: any) => {
    ...
    try {
      ...
      const providerOrSigner = filtered === 'write'
        ? new providers.Web3Provider(ethereum).getSigner()
        : provider;
      const contract = new ethers.Contract(contactAddress, [method], providerOrSigner);
      ...
      const toSend = method?.stateMutability === "payable"
        ? [...Object.values(sortKeysInObjectByRightParamsSequence(input, method)), {value: paybleValue}]
        : [...Object.values(sortKeysInObjectByRightParamsSequence(input, method))];
