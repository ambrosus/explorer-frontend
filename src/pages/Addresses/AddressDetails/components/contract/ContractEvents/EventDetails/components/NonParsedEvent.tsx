...
export function NonParsedEvent({ event, handleFilter }: { event: IEvent, handleFilter: any }) {
  ...
  return (
    <div>
      <div className="contract_events-body-transfer">
        ...
        <button className="universall_filter-btn" onClick={(e: any) => handleFilter(e, event.topics[0])}>
          <FilterIcon/>
        </button>
...
function Topic({ topic, name }: { topic: string, name: string }) {
  return (
    <div className="contract_events-body-cell" style={{marginLeft: "30px"}}>
      ...
      <pre className={"universall_light3"} style={{overflow: "hidden"}}>{topic}</pre>
    </div>
  )
}
